...
class WoaiwojiaSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=r'/ershoufang/\d+.html$'), callback='parse_detail', follow=True),
    )
    ...
    def parse_list(self, response):
        self.print_info(response, title='parse_list')
    ...
    def parse_detail(self, response):
        self.print_info(response, title='parse_detail')
    ...
    def print_info(self, response, title):
        print 'Spider ' + title + ' response successfully, ' + response.url + ', depth: ' + str(
            response.meta['depth']) + ', referer: ' + response.request.headers['Referer']
        # self.write_html_to_file(data=response.text, title=title)
